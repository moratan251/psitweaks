...
public final class PortableSpellProgrammerGameTests {
    ...
    public static void emptySelectedCadSocketDoesNotReportSuccess(GameTestHelper helper) {
        ...
        var cad = ItemCAD.makeCAD(new ItemStack(ModItems.cadAssemblyPsimetal),
        ...
        Consumer<PlayLevelSoundEvent.AtPosition> listener = event -> {
            if (event.getLevel() == helper.getLevel() && event.getSound() != null
                    && event.getSound().value() == PsiSoundHandler.bulletCreate) successSounds.incrementAndGet();
        };
        ...
        try {
            ...
            sockets.setBulletInSocket(0, new ItemStack(ModItems.spellBullet));
            assertMissingSelectedBullet(helper, player, programmer, cad);
            ...
            sockets.setBulletInSocket(1, new ItemStack(ModItems.spellBullet));
    ...
    public static void failedRegistrationExplainsReasonInActionBar(GameTestHelper helper) {
        ...
        var bullet = new ItemStack(ModItems.spellBullet);
    ...
    public static void itemCopiesAndDiskReloadRetainIndependentSpells(GameTestHelper helper) throws Exception {
        ...
        try {
            ...
            root.put("Item", second.save(new CompoundTag()));
            NbtIo.writeCompressed(root, file.toFile());
            var restored = ItemStack.of(NbtIo.readCompressed(file.toFile()).getCompound("Item"));
            ...
            helper.assertTrue(new SpellCompiler().compile(loaded).left().isPresent(), "Reloaded spell cannot compile");
        } finally {
    ...
    public static void offhandSingleAndStackedBulletsUsePsiRegistration(GameTestHelper helper) {
        ...
        var stacked = new ItemStack(ModItems.spellBullet, 3);
        ...
        for (ItemStack stack : player.getInventory().items) {
            if (stack.is(ModItems.spellBullet) && ItemSpellDrive.getSpell(stack) != null) {
    ...
    public static void openingAndEditingPayloadsRetainSpellConfiguration(GameTestHelper helper) {
        ...
        try {
            ...
            menu = new PortableSpellProgrammerMenu(9, player.getInventory(), 40, session, ItemPortableSpellProgrammer.getSpellCopy(held));
            ...
            message.write(buf);
            var decoded = MessagePortableSpellProgrammerEdit.read(buf);
            helper.assertTrue(decoded.containerId() == 9 && decoded.session().equals(session) && ItemPortableSpellProgrammer.spellTag(decoded.spell()).equals(ItemPortableSpellProgrammer.spellTag(message.spell())),
                    "Edit payload round trip changed the spell");
            helper.assertTrue(new SpellCompiler().compile(decoded.spell()).left().isPresent(),
    ...
    private static Spell spell(String name) {
        var spell = new Spell();
        spell.name = name;
        var trick = new vazkii.psi.common.spell.trick.PieceTrickDie(spell);
        var number = new vazkii.psi.common.spell.constant.PieceConstantNumber(spell);
        trick.x = 4;
        trick.y = 4;
        trick.isInGrid = true;
