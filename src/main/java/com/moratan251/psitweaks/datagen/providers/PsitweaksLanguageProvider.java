...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "macro_face_axial_offset", "Moves a position relative to the block face hit by a normal raycast from the caster's eyes. Position is optional; when omitted, the block position from that same raycast is used. Positive Left/Right moves right as the face is viewed, positive Forward/Backward moves into the block, and positive Up/Down moves toward the top of the view; negative values move in the opposite directions. On side faces, Up/Down follows world up. On the top and bottom faces, the caster's horizontal facing determines the rotation within the face. Other unconnected parameters are treated as 0. The mode button switches to one vector whose components are X = right, Y = up, and Z = into the block. The raycast uses the spell's maximum distance and a miss raises a null-vector error.", "術者の目からの通常レイキャストが命中したブロック面を基準に位置を移動します. 位置入力は任意で、省略時はその同じレイキャストが命中したブロック座標を使います. 左右の正の値は面を見た時の右へ、前後の正の値はブロック内側へ、上下の正の値は画面上へ移動し、負の値は逆方向です. 側面では上下がワールドの上下に一致します. 上面と下面では、術者の水平向きによって面内の回転を決めます. その他の未接続パラメータは 0 として扱われます. モードボタンでベクトル入力に切り替えると、X=右、Y=上、Z=ブロック内側の距離を成分とするベクトル1つで指定できます. レイキャスト距離は術式の最大距離で、命中しなかった場合は null vector エラーになります.");
