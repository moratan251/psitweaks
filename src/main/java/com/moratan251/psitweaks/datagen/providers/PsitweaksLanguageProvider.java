...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "trick_jump_flex", "Uses the same condition, label, and forward-only destination rules as Trick: Jump, then refunds the Psi cost of the spell pieces actually skipped by the jump. Pieces retained as dependencies of later actions are not refunded. With high-frequency casting, the client-side Psi display may temporarily desync.", "作動式: ジャンプと同じ条件, ラベル, 前方限定の移動規則で動作し, ジャンプによって実際にスキップしたスペルピースのPsiコストを返却します. 後続アクションの依存先として残るピースは返却対象になりません. 高頻度詠唱ではクライアント側のPsi表示が一時的にずれることがあります.");
