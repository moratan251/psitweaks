...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPieces(JsonObject root) {
        ...
        addSpellPiece(root, "operator_weak_raycast", "Operator: Vector Raycast (Weak)", "Raycasts from Position in the Ray direction and returns the coordinates of the hit block or fluid.", "演算子: ベクトルレイキャスト (弱)", "位置から Ray 方向へレイキャストし、命中したブロックまたは流体の座標を返します。");
        addSpellPiece(root, "operator_weak_raycast_axis", "Operator: Vector Raycast Direction (Weak)", "Raycasts from Position in the Ray direction and returns the normal direction of the face of the first block or fluid hit.", "演算子: ベクトルレイキャスト方向 (弱)", "位置から Ray 方向へレイキャストし、最初に命中したブロックまたは流体の面の法線方向を返します。");
        addSpellPiece(root, "operator_strong_raycast", "Operator: Vector Raycast (Strong)", "Raycasts from Position in the Ray direction, ignores blocks without a collision shape, and returns the coordinates of the hit block.", "演算子: ベクトルレイキャスト (強)", "位置から Ray 方向へレイキャストし、当たり判定のないブロックを無視して命中したブロックの座標を返します。");
        ...
        addSpellPiece(root, "macro_caster_raycast_axis", "Macro: Caster Raycast Direction", "Raycasts from the caster's position in the look direction and returns the normal direction of the hit face.", "マクロ: 術者レイキャスト方向", "術者の位置から視線方向へレイキャストし、命中した面の法線方向を返します。");
        addSpellPiece(root, "macro_caster_weak_raycast", "Macro: Caster Raycast (Weak)", "Like Macro: Caster Raycast, but it also hits fluids such as water and lava.", "マクロ: 術者レイキャスト (弱)", "マクロ: 術者レイキャストと同じですが、水や溶岩などの流体にも命中します。");
