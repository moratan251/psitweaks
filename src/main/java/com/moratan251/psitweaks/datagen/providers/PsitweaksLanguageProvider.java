...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "operator_string_length", "Returns the number of Unicode characters in the input String. Supplementary characters such as emoji count as one character.", "入力した String のUnicode文字数を Number として返します. 絵文字などのサロゲートペアは1文字として数えます.");
        addBookPage(root, "operator_string_replace", "Replaces every literal, case-sensitive occurrence of String 2 in String 1 with String 3. String 2 is not interpreted as a glob or regular expression. If String 2 is empty, String 1 is returned unchanged. The result is limited by the runtime String length limit.", "文字列1のうち, 文字列2と大文字小文字を区別してリテラル一致するすべての部分を文字列3へ置換します. 文字列2はglobや正規表現として解釈しません. 文字列2が空の場合は文字列1をそのまま返します. 結果は実行時のString文字数上限に従います.");
