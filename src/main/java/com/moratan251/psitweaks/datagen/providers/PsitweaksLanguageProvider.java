...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPieces(JsonObject root) {
        ...
        addSpellPiece(root, "operator_equal", "Operator: Equal", "Outputs 1 if Value 1 equals Value 2, otherwise 0. Any input type is accepted.", "演算子: 等しい", "値1と値2が等しいなら1、そうでなければ0を出力します。Any 型として入力できます。");
        ...
        addSpellPiece(root, "operator_to_string", "Operator: To String", "Converts an Any input into a String. Lists are converted element-by-element and joined with commas.", "演算子: 文字列へ変換", "Any 入力を String に変換します。List は各要素を変換し、コンマ区切りで結合します。");
        addSpellPiece(root, "operator_list_to_string_list", "Operator: To String List", "Converts an Any input into a String List. List inputs are converted element-by-element; non-list inputs become a one-element String List.", "演算子: 文字列リストへ変換", "Any 入力を String List に変換します。List は各要素を変換し、List 以外は1要素の String List になります。");
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "operator_equal", "Compares Value 1 and Value 2 as Any inputs. It outputs 1 when both values are equal, otherwise 0. Numbers compare by numeric value, Vectors compare by coordinates, and Item values compare their item data and source.", "値1と値2を Any 入力として比較します. 2つの値が等しいなら1, そうでなければ0を出力します. Number は数値, Vector は座標, Item はアイテムデータと source を比較します.");
        ...
        addBookPage(root, "operator_from_string", "Converts the String input into a Number or Vector for the selected mode. Number mode parses finite numeric text and outputs 0 on invalid input. Vector mode accepts Vector[x,y,z], Vector(x,y,z), [x,y,z], or (x,y,z), case-insensitive for Vector, and outputs the zero vector when parsing fails. Left-click the spell piece to select the mode.", "選択中のモードに応じて String 入力を Number または Vector に変換します. Number モードは有限の数値文字列を解析し, 無効な入力では0を出力します. Vector モードは Vector[x,y,z], Vector(x,y,z), [x,y,z], (x,y,z) を受け付けます. Vector は大文字小文字を区別せず, 解析失敗時はゼロベクトルを出力します. スペルピースを左クリックでモードを選択できます.");
        addBookPage(root, "operator_list_from_string_list", "Converts a String List into a Number List or Vector List for the selected mode. Number mode parses each entry as finite numeric text, so invalid entries become 0. Vector mode converts only entries matching Vector[x,y,z], Vector(x,y,z), [x,y,z], or (x,y,z), case-insensitive for Vector, and skips invalid entries. Left-click the spell piece to select the mode.", "選択中のモードに応じて String List を Number List または Vector List に変換します. Number モードは各要素を有限の数値文字列として解析し, 無効な要素は0になります. Vector モードは Vector[x,y,z], Vector(x,y,z), [x,y,z], (x,y,z) に一致する要素のみ変換し, 無効な要素は無視します. Vector は大文字小文字を区別しません. スペルピースを左クリックでモードを選択できます.");
        addBookPage(root, "operator_to_string", "Converts an Any input into a String. Entity and Item values output registry IDs, Number and Vector values use the same text form shown by Trick: Debug, and String values are returned as-is. List values convert each element by the same rules and join them with commas.", "Any 入力を String に変換します. Entity と Item はレジストリIDを出力し, Number と Vector は 作動式: デバッグ と同じ文字列表現を使い, String はそのまま返します. List は各要素を同じ規則で変換し, コンマ区切りで結合します.");
        addBookPage(root, "operator_list_to_string_list", "Converts an Any input into a String List. Entity and Item values output registry IDs, Number and Vector values use the same text form shown by Trick: Debug, and String values are returned as-is. List inputs convert each element by the same rules while preserving order. Non-list inputs become a one-element String List.", "Any 入力を String List に変換します. Entity と Item はレジストリIDを出力し, Number と Vector は 作動式: デバッグ と同じ文字列表現を使い, String はそのまま返します. List は入力順を維持して各要素を同じ規則で変換します. List 以外は1要素の String List になります.");
