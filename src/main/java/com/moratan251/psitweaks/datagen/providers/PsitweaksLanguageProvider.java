...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPiecesBook(JsonObject root) {
        addBookPage(root, "trick_idea_storage_view.0",
                "Opens your personal Ideaspace Storage. No inputs are required. Items, fluids, and Mekanism chemicals are stored separately, and your contents persist across logout and dimension changes. This feature is still under development.$(p)By default, it holds 256 item types, 64 fluid types, and 64 chemical types. Capacity limits can be changed in the config.",
        ...
        addBookPage(root, "trick_idea_storage_view.1",
                ...
                "アイテムをカーソルで持ち、ストレージ欄を左クリックすると格納します。インベントリ内のアイテムをShiftクリックしても格納できます。$(p)カーソルが空のとき、保存済みアイテムを左クリックすると最大1スタック、右クリックすると最大スタック数の半分まで取り出します。Shiftを押しながら操作すると、直接インベントリへ取り出します。");
