...
public class PsitweaksLanguageProvider extends LanguageProvider {
    ...
    private void addEnglishTranslations() {
        ...
        add("psi.book.page.psitweaks_overview.1", "$(item)Psionic Control Circuits$(0) are used as a bridge between Psi materials and machine-style crafting. If a recipe looks technological but still clearly belongs to spellcraft, this component is often involved.");
        ...
        add("psi.book.entry.psitweaks_spell_bullets", "Advanced Spell Bullets");
        add("psi.book.page.psitweaks_spell_bullets.0", "$(thing)PsiTweaks$(0) adds several tiers above the ordinary $(item)Spell Bullet$(0). These bullets are meant for stronger or more specialized CAD setups, and many bullet shapes have upgraded variants.");
        add("psi.book.page.psitweaks_spell_bullets.1", "The base upgraded bullets form the progression ladder. Shape-specific versions such as projectile, mine, loopcast, grenade, circle, and charge variants follow the same tier idea.");
        add("psi.book.page.psitweaks_spell_bullets.2", "Higher tiers are not just cosmetic. They are intended to sit alongside late-game CAD materials and more demanding spell setups.$(p)When building a combat or utility kit, treat bullet tier as part of the whole casting system: CAD stats, Psi capacity, regeneration, and spell cost all matter.");
        add("psi.book.entry.psitweaks_machines", "Psionic Machinery");
        ...
        add("psi.book.page.psitweaks_item.philosophers_stone", "A reusable catalyst for transmutation recipes. It remains in the crafting grid after use, letting it convert metals and other resources repeatedly without being consumed.");
        ...
        add("psi.book.page.psitweaks_item.flash_ring", "Stores a single spell and can cast it directly while you still use your normal CAD for power and stats.$(p)Sneak-use opens its spell editing screen. Once a spell is stored, the ring is named after that spell.");
        add("psi.book.page.psitweaks_item.portable_cad_assembler", "A handheld CAD Assembler. Use it to open an assembler interface without placing the block, useful for changing CAD parts or loaded bullets while away from base.");
        add("psi.book.page.psitweaks_item.sorcery_booster", "Equips in the Magic Calculation Area slot and increases spell damage by 30%.$(p)It is made from a $(item)Magician's Brain$(0), making it a combat-focused caster upgrade.");
        ...
        add("psi.book.page.psitweaks_item.inline_casters.1", "The Inline Caster has one slot, the Secondary Caster has five, and the Parallel Caster has eleven.$(p)Use the higher tiers when you want several spell bullets available without constantly changing the CAD.");
        add("psi.book.page.psitweaks_item.auto_casters.0", "Auto Casters are Magic Calculation Area curios that cast their selected spell automatically from Psi armor-style events.$(p)They still rely on your CAD for casting power and use the bullet socketed into the device.");
        add("psi.book.page.psitweaks_item.auto_casters.1", "The tick model fires every tick, the second model fires roughly once per second, and the custom tick model lets you set an interval from 1 to 1200 ticks by using the item.");
        add("psi.book.page.psitweaks_item.curios_controller", "Controls the selected bullet slot of socketable items equipped in Magic Calculation Area slots.$(p)It is the Curios-side counterpart to Psi's exosuit controller, useful when several caster curios are equipped.");
        ...
        add("psi.book.page.psitweaks_material.psionic_echo", "Made by infusing an $(item)Echo Shard$(0) with Psionic Echo infusion, or by using the $(l:psitweaks_spell_pieces/trick_supreme_infusion)$(o)Trick: Supreme Infusion$(0)$(/l).$(p)Used as the base catalyst for echo-tier alloys, HDΨE materials, gas processing, and advanced psionic machinery.");
        add("psi.book.page.psitweaks_material.chaotic_factor", "$(item)Chaotic Factor$(0) starts with infusing an $(item)Ender Pearl$(0) with Psigem to make $(item)Psionic Factor$(0). Infuse that with Ivory or Ebony to make the aligned factor variants, then cross the opposite alignment to create $(item)Chaotic Factor$(0).$(p)Used as the infusion source for $(item)Chaotic Psimetal$(0), the first major PsiTweaks metal above ordinary Psimetal.");
        add("psi.book.page.psitweaks_material.chaotic_psimetal", "Made by infusing ordinary $(item)Psimetal$(0) with Chaotic Factor.$(p)Used in recipes for Chaotic Psimetal CAD assemblies and $(item)Unrefined Flashmetal$(0).");
        add("psi.book.page.psitweaks_material.alloy_psion", "Made by infusing Mekanism's Atomic Alloy with Psigem.$(p)Used for $(item)Psionic Control Circuits$(0), Psionic Alloy CAD assemblies, and the next alloy tier.");
        add("psi.book.page.psitweaks_material.alloy_psionic_echo", "Made by infusing $(item)Psionic Alloy$(0) with Psionic Echo.$(p)Used for $(item)Echo Control Circuits$(0) and as the base for Hypostasis Alloy.");
        add("psi.book.page.psitweaks_material.alloy_hypostasis", "Made by infusing $(item)Echo Alloy$(0) with Hypostasis.$(p)Used for $(item)Hypostasis Control Circuits$(0) and high-tier psionic machinery.");
        add("psi.book.page.psitweaks_material.psionic_control_circuit", "Crafted from $(item)Psionic Alloy$(0) and a Mekanism Ultimate Control Circuit.$(p)Used in PsiTweaks machines, upgraded spell bullets, and similar recipes.");
        add("psi.book.page.psitweaks_material.echo_control_circuit", "Crafted from $(item)Psionic Control Circuit$(0), $(item)Echo Alloy$(0), and HDΨE Sheets.$(p)Used in recipes for advanced spell bullets and machinery.");
        add("psi.book.page.psitweaks_material.hypostasis_control_circuit", "Crafted from $(item)Echo Control Circuit$(0), $(item)Hypostasis Alloy$(0), and HDΨE Sheets.$(p)Used for the highest machinery.");
        add("psi.book.page.psitweaks_material.echo_pellet", "A compact HDΨE material made from Psionic Echo processing.$(p)Used where recipes need condensed echo material rather than raw Psionic Echo. The ΨE-O Fuel produced as a byproduct can be used as fuel for the Gas Generator.");
        ...
        add("psi.book.page.psitweaks_material.magicians_brain", "Dropped when a Magician villager is killed by $(l:psitweaks_spell_pieces/trick_guillotine)$(o)Trick: Guillotine$(0)$(/l).$(p)Used for the $(item)Sorcery Booster$(0) and the Phenomenon Interference Enhancement Unit module.");
        add("psi.book.page.psitweaks_material.jade", "Made by applying material mutation to an $(item)Emerald Block$(0), either with the spell trick or the Material Mutator.$(p)Used as the main material for crafting $(item)Magatama$(0).");
        add("psi.book.page.psitweaks_material.hypostasis_gem", "Made by applying material mutation to an $(item)Antinite Block$(0), either with the spell trick or the Material Mutator.$(p)It can be enriched and converted into Hypostasis infusion, and is also used for $(item)Magatama$(0), Hypostasis Alloy, and the Psycheonic Metal chain.");
        add("psi.book.page.psitweaks_material.magatama", "Crafted from $(item)Jade$(0) surrounding a $(item)Hypostasis Gem$(0).$(p)Used in Spellmachinery Casings, making it a bridge between material mutation and advanced psionic machinery.");
        add("psi.book.page.psitweaks_material.unrefined_flashmetal", "Crafted from $(item)Chaotic Psimetal$(0) and Refined Glowstone.$(p)Enrich it to obtain usable $(item)Flashmetal$(0).");
        add("psi.book.page.psitweaks_material.flashmetal", "Made by enriching $(item)Unrefined Flashmetal$(0).$(p)Used for Flashmetal blocks, Flashmetal CAD assemblies, Heavy Psimetal, and sublimated-tier spell bullets.");
        add("psi.book.page.psitweaks_material.heavy_psimetal_scrap", "Made by infusing Netherite Scrap with Psionic Echo.$(p)Used with $(item)Flashmetal$(0) to craft $(item)Heavy Psimetal$(0).");
        add("psi.book.page.psitweaks_material.heavy_psimetal", "Crafted from $(item)Heavy Psimetal Scrap$(0) and $(item)Flashmetal$(0).$(p)Used for stronger CAD assemblies, gear, and the Psycheonic Metal chain.");
        add("psi.book.page.psitweaks_material.psycheonic_metal_ingot", "Infuse $(item)Heavy Psimetal$(0) with Hypostasis to make $(item)Psycheonic Metal Nuggets$(0), then craft nine nuggets into an ingot.$(p)Used for transcendent spell bullets, Psycheonic Metal CAD assemblies, and the highest psionic material tier.");
        add("psi.book.page.psitweaks_material.antinite_ingot", "Smelted from Antinite ore or recovered from Antinite blocks. The dust, shard, clump, crystal, and dirty dust forms are intermediate products in Mekanism-style ore processing.$(p)Used as a material for Hypostasis Gems and the $(item)Philosopher's Stone$(0).");
    ...
    private void addJapaneseTranslations() {
        ...
        add("psi.book.page.psitweaks_overview.1", "$(item)サイオニック制御回路$(0) は, Psi素材と機械的なクラフトをつなぐ部品です. 技術的に見えるが術式工学に属するレシピでは, この部品がよく使われます.");
        ...
        add("psi.book.entry.psitweaks_spell_bullets", "高位術式弾");
        add("psi.book.page.psitweaks_spell_bullets.0", "$(thing)PsiTweaks$(0) は通常の$(item)術式弾$(0)より上位の複数ティアを追加します. これらはより強力または専門的なCAD構成向けの術式弾で, 多くの弾種に高位版があります.");
        add("psi.book.page.psitweaks_spell_bullets.1", "基本の高位術式弾は進行段階の軸になります. 発射型, 地雷型, ループ型, 榴弾型, 円形, チャージ型などの派生弾も同じティア構造に従います.");
        add("psi.book.page.psitweaks_spell_bullets.2", "高位ティアは見た目だけの違いではありません. 終盤のCAD素材や, より要求の大きい術式構成と組み合わせることを想定しています.$(p)戦闘用や汎用の構成を作るときは, 術式弾のティアも詠唱システム全体の一部として扱ってください. CAD性能, Psi保有量, 回復速度, 術式コストのすべてが重要です.");
        add("psi.book.entry.psitweaks_machines", "サイオニック機械");
        ...
        add("psi.book.page.psitweaks_machines.2", "$(item)サイリンク発電機$(0) は Mekanism の所有者にリンクします. 有効化され, チャンクが読み込まれ, 所有者がオンラインのとき, そのプレイヤーのPsiを消費して内部バッファへエネルギーを生成します.$(p)発電レートはGUIで設定できます. tickあたりのPsi消費を増やすほど出力も増えますが, 所有者のPsiも速く減ります.");
        ...
        add("psi.book.page.psitweaks_item.philosophers_stone", "変換レシピで使う再利用可能な触媒です. クラフト後も作業台に残るため, 金属や一部資源の変換に繰り返し使えます.");
        ...
        add("psi.book.page.psitweaks_item.flash_ring", "単体の術式を保存し, 通常のCADを出力や性能の基盤として使いながら直接詠唱できます.$(p)スニーク使用で術式編集画面を開きます. 術式を保存すると, リング名はその術式名になります.");
        add("psi.book.page.psitweaks_item.portable_cad_assembler", "手持ちで使えるCAD組立機です. ブロックを設置せずに組立機画面を開けるため, 拠点外でもCAD部品や装填済み術式弾を調整できます.");
        add("psi.book.page.psitweaks_item.sorcery_booster", "魔法演算領域スロットに装備し, 術式ダメージを30%増加させます.$(p)$(item)魔法師の脳$(0) を使う, 戦闘向けの魔法師強化装備です.");
        ...
        add("psi.book.page.psitweaks_item.inline_casters.1", "インラインキャスターは1スロット, セカンダリキャスターは5スロット, パラレルキャスターは11スロットです.$(p)CADを頻繁に差し替えず, 複数の術式弾を持ち替えたいときに使います.");
        add("psi.book.page.psitweaks_item.auto_casters.0", "術式自動詠唱デバイスは, Psi防具のイベントに似たタイミングで選択中の術式を自動詠唱する魔法演算領域用Curiosです.$(p)詠唱性能には装備中のCADを使い, デバイス内の術式弾を実行します.");
        add("psi.book.page.psitweaks_item.auto_casters.1", "tick型は毎tick, セカンド型はおおよそ1秒ごとに詠唱します. カスタムtick型は, アイテム使用で1から1200tickの範囲から詠唱間隔を設定できます.");
        add("psi.book.page.psitweaks_item.curios_controller", "魔法演算領域スロットに装備したソケット対応アイテムの選択術式弾スロットを操作します.$(p)複数の詠唱Curiosを装備しているときに使う, Curios版の防具コントローラです.");
        ...
        add("psi.book.page.psitweaks_material.psionic_echo", "$(item)残響の欠片$(0) にサイオニックエコーを注入するか, $(l:psitweaks_spell_pieces/trick_supreme_infusion)$(o)作動式: 超位注入$(0)$(/l)で作成します.$(p)感応系合金, HDΨE 素材, 気体加工, 上位のサイオニック機械の基礎触媒です.");
        add("psi.book.page.psitweaks_material.chaotic_factor", "$(item)カオティック因子$(0) は, $(item)エンダーパール$(0) にサイジェムを注入して $(item)サイオニック因子$(0) を作るところから始まります. そこへアイボリーまたはエボニーを注入して偏向因子を作り, 反対属性を重ねると $(item)カオティック因子$(0) になります.$(p)通常のサイメタルを上回る最初の PsiTweaks 金属, $(item)カオティックサイメタル$(0) の注入源です.");
        add("psi.book.page.psitweaks_material.chaotic_psimetal", "通常の $(item)サイメタル$(0) にカオティック因子を注入して作ります.$(p)カオティックサイメタル CAD 素体や $(item)未精製フラッシュメタル$(0) のレシピに使います.");
        add("psi.book.page.psitweaks_material.alloy_psion", "Mekanism の原子合金にサイジェムを注入して作ります.$(p)$(item)サイオニック制御回路$(0), サイオニック合金 CAD 素体, 次 tier の合金素材に使います.");
        add("psi.book.page.psitweaks_material.alloy_psionic_echo", "$(item)サイオニック合金$(0) にサイオニックエコーを注入して作ります.$(p)$(item)感応制御回路$(0) や位格合金の素材に使います.");
        add("psi.book.page.psitweaks_material.alloy_hypostasis", "$(item)感応合金$(0) にヒュポスタシスを注入して作ります.$(p)$(item)位格制御回路$(0) や高 tier のサイオニック機械に使います.");
        add("psi.book.page.psitweaks_material.psionic_control_circuit", "$(item)サイオニック合金$(0) と Mekanism の究極制御回路からクラフトします.$(p)PsiTweaks の機械, 強化術式弾などのレシピに使います.");
        add("psi.book.page.psitweaks_material.echo_control_circuit", "$(item)サイオニック制御回路$(0), $(item)感応合金$(0), HDΨE シートからクラフトします.$(p) 上位の術式弾や機械のレシピに使います.");
        add("psi.book.page.psitweaks_material.hypostasis_control_circuit", "$(item)感応制御回路$(0), $(item)位格合金$(0), HDΨE シートからクラフトします.$(p)最上位の機械に使います.");
        add("psi.book.page.psitweaks_material.echo_pellet", "サイオニックエコー加工から得られる圧縮 HDΨE 素材です.$(p)生のサイオニックエコーではなく, 凝縮した感応素材を要求するレシピに使います. 副産物として得られるΨE-O燃料はガス発電機の燃料として使えます.");
        add("psi.book.page.psitweaks_material.echo_sheet", "HDΨE 素材をシート状に濃縮して作ります.$(p)感応制御回路と位格制御回路のレシピで多く使います.");
        add("psi.book.page.psitweaks_material.magicians_brain", "魔法師の村人を $(l:psitweaks_spell_pieces/trick_guillotine)$(o)作動式: ギロチン$(0)$(/l) で倒したときにドロップします.$(p)$(item)ソーサリーブースター$(0) と事象干渉力増強ユニットの素材に使います.");
        add("psi.book.page.psitweaks_material.jade", "$(item)エメラルドブロック$(0) に物質変成を作用させて作ります. 術式でも物質変成機でも入手できます.$(p)$(item)勾玉$(0) を作るための主素材です.");
        add("psi.book.page.psitweaks_material.hypostasis_gem", "$(item)アンティナイトブロック$(0) に物質変成を作用させて作ります. 術式でも物質変成機でも入手できます.$(p)濃縮してヒュポスタシス注入へ変換できるほか, $(item)勾玉$(0), 位格合金, プシオニックメタル系列に使います.");
        add("psi.book.page.psitweaks_material.magatama", "$(item)翡翠$(0) で $(item)ヒュポスタシスジェム$(0) を囲んでクラフトします.$(p)スペル機械筐体に使うため, 物質変成素材と上位サイオニック機械をつなぐ素材です.");
        add("psi.book.page.psitweaks_material.unrefined_flashmetal", "$(item)カオティックサイメタル$(0) と精製グロウストーンからクラフトします.$(p)濃縮すると使用可能な $(item)フラッシュメタル$(0) になります.");
        add("psi.book.page.psitweaks_material.flashmetal", "$(item)未精製フラッシュメタル$(0) を濃縮して作ります.$(p)フラッシュメタルブロック, フラッシュメタル CAD 素体, ヘビーサイメタル, 昇華 tier の術式弾に使います.");
        add("psi.book.page.psitweaks_material.heavy_psimetal_scrap", "ネザライトの欠片にサイオニックエコーを注入して作ります.$(p)$(item)フラッシュメタル$(0) と組み合わせて $(item)ヘビーサイメタル$(0) を作ります.");
        add("psi.book.page.psitweaks_material.heavy_psimetal", "$(item)ヘビーサイメタルの欠片$(0) と $(item)フラッシュメタル$(0) からクラフトします.$(p)より強い CAD 素体, 装備, プシオニックメタル系列に使います.");
        add("psi.book.page.psitweaks_material.psycheonic_metal_ingot", "$(item)ヘビーサイメタル$(0) にヒュポスタシスを注入して $(item)プシオニックメタルナゲット$(0) を作り, 9個クラフトするとインゴットになります.$(p)超越 tier の術式弾, プシオニックメタル CAD 素体, 最上位のサイオニック素材に使います.");
        add("psi.book.page.psitweaks_material.antinite_ingot", "アンティナイトの鉱石を精錬するか, アンティナイトブロックから戻して入手します. 粉, 欠片, 凝塊, 結晶, 汚れた粉は Mekanism 式鉱石処理の中間素材です.$(p)ヒュポスタシスジェムや賢者の石の素材になります.");
