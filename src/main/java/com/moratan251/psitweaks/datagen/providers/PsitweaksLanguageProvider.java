...
public class PsitweaksLanguageProvider extends LanguageProvider {
    ...
    private void addEnglishTranslations() {
        ...
        add("psi.book.page.psitweaks_research.1", "Place the required ingredients in the $(item)Program Research Table$(0) and supply FE to craft a program.$(p)JEI lists each research recipe's required materials, energy cost, and processing time.");
        ...
        add("psi.book.page.psitweaks_mekanism_integration.1", "The $(item)Metallurgic Infuser$(0) can produce core Psi materials with PsiTweaks infuse types. Psigem infusion turns redstone into $(item)Psidust$(0) and gold into $(item)Psimetal$(0).$(p)Ebony and Ivory infusion can make $(item)Ebony Substance$(0), $(item)Ivory Substance$(0), and their Psimetal variants. Later recipes extend this chain into psionic factors, $(item)Chaotic Psimetal$(0), Psionic Echo materials, and Hypostasis-tier metal.");
        add("psi.book.page.psitweaks_mekanism_integration.2", "PsiTweaks adds three MekaSuit modules for Psi users. The $(item)Psyon Supplying Unit$(0) and $(item)Psyon Capacity Unit$(0) install in the MekaSuit body armor and improve Psi regeneration and maximum Psi.$(p)The $(item)Phenomenon Interference Enhancement Unit$(0) installs in the MekaSuit helmet and increases spell damage.");
        add("psi.book.entry.psitweaks_spell_bullets", "Upgraded Spell Bullets");
        add("psi.book.page.psitweaks_spell_bullets.0", "$(thing)PsiTweaks$(0) adds upgraded tiers above the ordinary $(item)Spell Bullet$(0): Advanced, Resonant, Sublimated, Awakened, and Transcendent.$(p)They are meant for stronger or more specialized CAD setups, and many bullet shapes have upgraded variants.");
        add("psi.book.page.psitweaks_spell_bullets.1", "The standard bullet variants form the core progression ladder. Projectile, mine, loopcast, grenade, circle, and charge variants follow the same tier structure.");
        add("psi.book.page.psitweaks_spell_bullets.2", "Upgraded tiers are not just cosmetic. They are intended to sit alongside late-game CAD materials and more demanding spell setups.$(p)When building a combat or utility kit, treat bullet tier as part of the whole casting system: CAD stats, Psi capacity, regeneration, and spell cost all matter.");
        ...
        add("psi.book.page.psitweaks_machine.material_mutator", "A Mekanism injecting-style machine that performs material mutation with $(item)Psionic Echo Gas$(0) and energy.$(p)It automates mutations that can also be produced by $(l:psitweaks_spell_pieces/trick_material_mutation)$(o)Trick: Material Mutation$(0)$(/l), such as Jade and Hypostasis Gems.");
        ...
        add("psi.book.page.psitweaks_item.blank_program", "Used as the base item for program research. Put it into the $(item)Program Research Table$(0) with the required materials to create a written program.$(p)It can also be crafted with an existing written program item to duplicate that program.");
        ...
        add("psi.book.page.psitweaks_material.pellet_neptunium", "Made by mutating a $(item)Polonium Block$(0) with $(l:psitweaks_spell_pieces/trick_material_mutation)$(o)Trick: Material Mutation$(0)$(/l), or by processing it in the $(l:psitweaks_machines/material_mutator)$(o)Material Mutator$(0)$(/l).$(p)Used for the $(item)Transcendent Universal Cable$(0) and the MekaSuit $(item)Phenomenon Interference Enhancement Unit$(0).");
    ...
    private void addJapaneseTranslations() {
        ...
        add("psi.book.page.psitweaks_research.1", "$(item)プログラム研究台$(0) に必要素材を入れて FE を供給すると, プログラムをクラフトできます.$(p)各研究に必要な素材, 消費電力, 処理時間は JEI で確認できます.");
        ...
        add("psi.book.page.psitweaks_mekanism_integration.1", "$(item)冶金吹き込み機$(0) では, PsiTweaks の注入タイプを使って主要なPsi素材を作れます. サイジェム注入はレッドストーンを$(item)サイダスト$(0)に, 金インゴットを$(item)サイメタル$(0)に変換します.$(p)エボニー注入とアイボリー注入では, $(item)エボニー基質$(0), $(item)アイボリー基質$(0), それぞれのサイメタル派生素材を作れます. さらにサイオニック因子, $(item)カオティックサイメタル$(0), サイオニックエコー素材, ヒュポスタシス系金属へ発展します.");
        add("psi.book.page.psitweaks_mekanism_integration.2", "PsiTweaks はPsi使い向けのMekaSuitモジュールを3種類追加します. $(item)サイオン供給ユニット$(0) と $(item)サイオン容量ユニット$(0) はMekaSuit胴体に装着し, Psi回復速度と最大Psi保有量を強化します.$(p)$(item)事象干渉力増大ユニット$(0) はMekaSuitヘルメットに装着し, 術式ダメージを増加させます.");
        add("psi.book.entry.psitweaks_spell_bullets", "強化術式弾");
        add("psi.book.page.psitweaks_spell_bullets.0", "$(thing)PsiTweaks$(0) は通常の$(item)術式弾$(0)より上位の強化ティアとして, 改良, 共鳴, 昇華, 覚醒, 超越を追加します.$(p)これらはより強力または専門的なCAD構成向けの術式弾で, 多くの弾種に強化版があります.");
        add("psi.book.page.psitweaks_spell_bullets.1", "標準型の強化術式弾は進行段階の軸になります. 発射型, 地雷型, ループ型, 榴弾型, 円形, チャージ型などの派生弾も同じティア構造に従います.");
        add("psi.book.page.psitweaks_spell_bullets.2", "強化ティアは見た目だけの違いではありません. 終盤のCAD素材や, より要求の大きい術式構成と組み合わせることを想定しています.$(p)戦闘用や汎用の構成を作るときは, 術式弾のティアも詠唱システム全体の一部として扱ってください. CAD性能, Psi保有量, 回復速度, 術式コストのすべてが重要です.");
        ...
        add("psi.book.page.psitweaks_machine.material_mutator", "$(item)気化サイオニックエコー$(0) と電力を使い, 物質変成を実行するMekanism注入形式の機械です.$(p)$(l:psitweaks_spell_pieces/trick_material_mutation)$(o)作動式: 物質変成$(0)$(/l)でも得られる翡翠やヒュポスタシスジェムなどの変成を自動化できます.");
        ...
        add("psi.book.page.psitweaks_item.blank_program", "プログラム研究の基礎素材です. $(item)プログラム研究台$(0) に必要素材と一緒に入れることで, 記入済みのプログラムを作成できます.$(p)既存の記入済みプログラムアイテムとクラフトすると, そのプログラムを複製することもできます.");
        ...
        add("psi.book.page.psitweaks_material.pellet_neptunium", "$(item)ポロニウムブロック$(0) を $(l:psitweaks_spell_pieces/trick_material_mutation)$(o)作動式: 物質変成$(0)$(/l) で変成するか, $(l:psitweaks_machines/material_mutator)$(o)物質変成機$(0)$(/l) で加工して作ります.$(p)$(item)超越ユニバーサルケーブル$(0) や MekaSuit の $(item)事象干渉力増大ユニット$(0) に使います.");
