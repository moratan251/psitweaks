...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPieces(JsonObject root) {
        ...
        addSpellPiece(root, "selector_indexed_element", "Selector: Indexed Element", "Outputs the element at a zero-based index from the selected List mode.", "取得子: インデックス要素", "選択中の List モードから0始まりのインデックスにある要素を取得します。");
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "selector_indexed_element", "Outputs the element at a zero-based index from the selected List mode. Available modes come from Psitweaks list adapters. Out-of-range indexes produce the same out-of-bounds spell error as Psi's Indexed Element.", "選択中の List モードから, 0始まりのインデックスにある要素を出力します. 利用可能なモードは Psitweaks の list adapter から追加されます. 範囲外のインデックスは Psi 本体の Indexed Element と同じ範囲外エラーになります.");
