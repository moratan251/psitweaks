...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPieces(JsonObject root) {
        ...
        addSpellPiece(root, "macro_face_axial_offset", "Macro: Face Axial Offset", "Offsets a position along axes aligned to the block face under the caster's crosshair. Positive values move screen-right, into the block, and screen-up.", "マクロ: 面基準平行移動", "術者の視線が命中したブロック面を基準に、位置を左右・前後・上下へ平行移動します。正の値で画面右・ブロック内側・画面上へ移動します。");
        addSpellPiece(root, "macro_face_axial_rotation", "Macro: Face Axial Rotation", "Rotates a direction vector from the coordinate frame of the block face under the caster's crosshair into world space (X = right, Y = up, Z = into the block).", "マクロ: 面基準回転", "方向ベクトルを、術者の視線が命中したブロック面の座標系(X=右、Y=上、Z=ブロック内側)からワールド座標へ回転します。");
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "macro_face_axial_offset", "Moves the given position relative to the block face hit by a normal raycast from the caster's eyes. Positive Left/Right moves right as the face is viewed, positive Forward/Backward moves into the block, and positive Up/Down moves toward the top of the view; negative values move in the opposite directions. On side faces, Up/Down follows world up. On the top and bottom faces, the caster's horizontal facing determines the rotation within the face. Unconnected parameters are treated as 0. The mode button switches to one vector whose components are X = right, Y = up, and Z = into the block. The raycast uses the spell's maximum distance and a miss raises a null-vector error.", "術者の目からの通常レイキャストが命中したブロック面を基準に、指定位置を移動します. 左右の正の値は面を見た時の右へ、前後の正の値はブロック内側へ、上下の正の値は画面上へ移動し、負の値は逆方向です. 側面では上下がワールドの上下に一致します. 上面と下面では、術者の水平向きによって面内の回転を決めます. 未接続のパラメータは 0 として扱われます. モードボタンでベクトル入力に切り替えると、X=右、Y=上、Z=ブロック内側の距離を成分とするベクトル1つで指定できます. レイキャスト距離は術式の最大距離で、命中しなかった場合は null vector エラーになります.");
        addBookPage(root, "macro_face_axial_rotation", "Rotates the given direction vector from the coordinate frame of the block face hit by a normal raycast from the caster's eyes into world space. X is right as the face is viewed, Y is toward the top of the view, and Z is into the block. On side faces, Y follows world up. On the top and bottom faces, the caster's horizontal facing determines the rotation within the face. The vector's magnitude is preserved. The raycast uses the spell's maximum distance and a miss raises a null-vector error.", "術者の目からの通常レイキャストが命中したブロック面の座標系から、指定した方向ベクトルをワールド座標へ回転します. Xは面を見た時の右、Yは画面上、Zはブロック内側です. 側面ではYがワールド上方向に一致します. 上面と下面では、術者の水平向きによって面内の回転を決めます. ベクトルの大きさは保存されます. レイキャスト距離は術式の最大距離で、命中しなかった場合は null vector エラーになります.");
