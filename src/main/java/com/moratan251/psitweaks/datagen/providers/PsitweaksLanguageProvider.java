...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPieces(JsonObject root) {
        ...
        addSpellPiece(root, "selector_nbt", "Selector: NBT", "Outputs the selected target's top-level NBT as key:value strings.", "取得子: NBT", "選択モードの対象のNBTトップレベルを key:value 形式の String List として出力します。");
        addSpellPiece(root, "selector_nbt_keys", "Selector: NBT Keys", "Outputs the selected target's top-level NBT keys as a String List.", "取得子: NBTキー", "選択モードの対象のNBTトップレベルキーを String List として出力します。");
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "selector_block", "Outputs the block at the target position as a Block value. Block is a read-only snapshot containing the saved position, dimension, block state, registry ID, and block tags. Block can also be connected to Vector and Plain Value inputs, where it is used as its saved position.", "対象座標にあるブロックを Block 型として出力します. Block は保存座標, ディメンション, ブロック状態, レジストリID, ブロックタグを持つ読み取り用スナップショットです. Block は Vector 入力や Plain Value 入力にも接続でき, その場合は保存座標として扱われます.");
        ...
        addBookPage(root, "operator_list_search", "Filters the selected List mode by comparing the String input with each element converted by Operator: To String. Contextual values use the same debug-display text as that operator. The String input accepts wildcards: * matches any text, ? matches one character, and [abc] matches one of the listed characters. Without wildcards, matching is exact.", "選択中の List モードを, 各要素を 演算子: 文字列へ変換 と同じ規則で文字列化した値で絞り込みます. Contextual Value は同じデバッグ表示相当の文字列で比較します. String入力ではワイルドカードを利用でき, * は任意の文字列, ? は任意の1文字, [abc] は a/b/c のいずれか1文字に一致します. ワイルドカードなしでは完全一致です.");
        addBookPage(root, "operator_list_search_exclude", "Filters the selected List mode by removing elements whose String form matches the input wildcard. Contextual values use the same debug-display text as Operator: To String. * matches any text, ? matches one character, and [abc] matches one of the listed characters. Without wildcards, matching is exact.", "選択中の List モードから, 文字列化した値が入力ワイルドカードに一致する要素を除外します. Contextual Value は 演算子: 文字列へ変換 と同じデバッグ表示相当の文字列で比較します. * は任意の文字列, ? は任意の1文字, [abc] は a/b/c のいずれか1文字に一致します. ワイルドカードなしでは完全一致です.");
