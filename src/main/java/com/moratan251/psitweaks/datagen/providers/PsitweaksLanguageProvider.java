...
public class PsitweaksLanguageProvider implements DataProvider {
    ...
    private void addSpellPieces(JsonObject root) {
        ...
        addSpellPiece(root, "operator_inside_region", "Operator: Inside Region", "Keeps only the entities in the input Entity List that are inside the matrix-defined parallelepiped.", "演算子: 領域内", "入力した Entity List のうち、行列で定義された並行六面体の内部にいるエンティティのみを残します。");
        addSpellPiece(root, "operator_outside_region", "Operator: Outside Region", "Keeps only the entities in the input Entity List that are outside the matrix-defined parallelepiped.", "演算子: 領域外", "入力した Entity List のうち、行列で定義された並行六面体の外部にいるエンティティのみを残します。");
        ...
        addSpellPiece(root, "trick_mass_block_break", "Trick: Mass Block Break", "Breaks blocks at the coordinates in a Vector List. Drops are intentionally collected into your inventory first; only overflow is dropped.", "作動式: 大規模ブロック破壊", "Vector List の座標にあるブロックを破壊します。ドロップは意図的に先にインベントリへ回収され、入りきらない分だけドロップします。");
    }
    ...
    private void addSpellPiecesBook(JsonObject root) {
        ...
        addBookPage(root, "operator_inside_region", "Filters an Entity List and returns only the entities whose center is inside the parallelepiped defined by a 3x4 or 4x4 matrix. The first three columns are the edge vectors and the fourth column is the start point. Entities exactly on the boundary are counted as inside.", "3×4 または 4×4 の行列で定義された並行六面体の内部に中心があるエンティティのみを残した Entity List を返します. 最初の3列は辺ベクトル、第4列は開始点です. 境界面上にあるエンティティも内部とみなします。");
        addBookPage(root, "operator_outside_region", "Filters an Entity List and returns only the entities whose center is outside the parallelepiped defined by a 3x4 or 4x4 matrix. The first three columns are the edge vectors and the fourth column is the start point. Entities exactly on the boundary are counted as inside, so they are excluded from the result.", "3×4 または 4×4 の行列で定義された並行六面体の外部に中心があるエンティティのみを残した Entity List を返します. 最初の3列は辺ベクトル、第4列は開始点です. 境界面上にあるエンティティは内部とみなすため、結果から除外されます。");
