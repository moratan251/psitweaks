...
public final class ConnectorTransfers {
    ...
    public static boolean push(IdeaspaceConnectorBlockEntity source, PlayerIdeaStorage storage, Direction direction, int dueSlots) {
        ...
        for (int slot = 0; slot < IdeaspaceConnectorBlockEntity.SLOTS; slot++) {
            ...
            switch (resource.kind()) {
                case ITEM -> {
                    if (!checkedItems) {
                        items = ConnectorCapabilities.get(level, targetPos, face, ForgeCapabilities.ITEM_HANDLER);
                        checkedItems = true;
                    }
                ...
                case FLUID -> {
                    if (!checkedFluids) {
                        fluids = ConnectorCapabilities.get(level, targetPos, face, ForgeCapabilities.FLUID_HANDLER);
                        checkedFluids = true;
                    }
                ...
                case ENERGY -> {
                    if (!checkedEnergy) {
                        energy = ConnectorCapabilities.get(level, targetPos, face, ForgeCapabilities.ENERGY);
                        checkedEnergy = true;
                    }
    ...
    public static int pushFluid(PlayerIdeaStorage storage, FluidResourceKey key, IFluidHandler target, int maximum) {
        ...
        if (available <= 0) return 0;
        FluidStack offered = new FluidStack(key.template(), available);
        ...
        try {
            if (withdrawal.amount() > 0) accepted = (int) Math.max(0, Math.min(withdrawal.amount(), target.fill(
                    new FluidStack(key.template(), (int) withdrawal.amount()), IFluidHandler.FluidAction.EXECUTE)));
            return accepted;
        } finally {
            withdrawal.restore(withdrawal.amount() - accepted);
        }
