...
public record ConnectorResource(Kind kind, ItemResourceKey item, FluidResourceKey fluid, ResourceLocation chemical) {
    ...
    public CompoundTag save() {
        ...
        switch (kind) {
            case ITEM -> tag.put("Item", item.save());
            case FLUID -> tag.put("Fluid", fluid.save());
    ...
    public static ConnectorResource load(CompoundTag tag) {
        return switch (tag.getString("Kind")) {
            case "ITEM" -> ItemResourceKey.parse(tag.getCompound("Item"))
                    .map(ConnectorResource::item).orElse(EMPTY);
            case "FLUID" -> FluidResourceKey.parse(tag.getCompound("Fluid"))
    ...
    public FluidStack fluidStack(int amount) {
        return fluid == null || amount <= 0 ? FluidStack.EMPTY : new FluidStack(fluid.template(), amount);
    }
}
