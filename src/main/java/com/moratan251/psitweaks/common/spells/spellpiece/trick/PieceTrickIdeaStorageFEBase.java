...
public abstract class PieceTrickIdeaStorageFEBase extends PieceTrick {
    ...
    public Object execute(SpellContext context) throws SpellRuntimeException {
        Vector3 pos = getParamValue(context, position);
        Vector3 face = getParamValue(context, direction);
        Number powerValue = getParamValue(context, power);
        if (pos == null || face == null || powerValue == null) throw new SpellRuntimeException(SpellRuntimeException.NULL_VECTOR);
        double strength = powerValue.doubleValue();
        ...
        var handler = ConnectorCapabilities.get(level, blockPos, Direction.getNearest(face.x, face.y, face.z), ForgeCapabilities.ENERGY);
