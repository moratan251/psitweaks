...
public class PieceTrickDispelBeneficial extends PieceTrick {
    SpellParam<Entity> target;

    public PieceTrickDispelBeneficial(Spell spell) {
        super(spell);
        ...
        if (!(targetVal instanceof LivingEntity)) {
            ...
            for (MobEffectInstance effect : effects) {
                if (effect.getEffect().value().isBeneficial()) {
                    ((LivingEntity) targetVal).removeEffect(effect.getEffect());
                }
            }
