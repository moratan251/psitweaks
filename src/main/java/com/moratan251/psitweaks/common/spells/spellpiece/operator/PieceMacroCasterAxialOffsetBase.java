...
abstract class PieceMacroCasterAxialOffsetBase extends PieceOperator implements PsitweaksModeConfigurable {
    ...
    public Object execute(SpellContext context) throws SpellRuntimeException {
        ...
        if (isVectorMode()) {
            ...
            if (offsetValue != null) {
                addBasisOffset(result, basis, offsetValue.x, offsetValue.z, offsetValue.y);
            }
            return result;
        }

        double leftRightValue = numberOrZero(getParamValue(context, leftRight));
        double forwardBackwardValue = numberOrZero(getParamValue(context, forwardBackward));
        double upDownValue = numberOrZero(getParamValue(context, upDown));
    ...
    private static void addBasisOffset(Vector3 result, CasterAxialBasis basis,
                                       double leftRight, double forwardBackward, double upDown) {
        if (leftRight != 0) {
