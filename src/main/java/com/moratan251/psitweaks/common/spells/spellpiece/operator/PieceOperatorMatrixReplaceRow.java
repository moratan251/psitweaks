...
public class PieceOperatorMatrixReplaceRow extends PsitweaksPieceOperator {
    private SpellParam<MatrixValue> matrix;
    private SpellParam<Number> row;
    private SpellParam<Object> values;

    public PieceOperatorMatrixReplaceRow(Spell spell) {
        super(spell);
    }
    ...
    public Object execute(SpellContext context) throws SpellRuntimeException {
        ...
        int index = getNotNullParamValue(context, row).intValue();
        var list = MatrixOperations.asNumberList(getNotNullParamValue(context, values));
        return MatrixOperations.replaceRow(mat, index, list);
    }
