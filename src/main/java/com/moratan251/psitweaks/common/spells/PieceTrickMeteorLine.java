...
public class PieceTrickMeteorLine extends PieceTrick {
    ...
    public Object execute(SpellContext context) throws SpellRuntimeException {
        ...
        Number maxDistanceValue = getParamValue(context, maxDistance);
        ...
        Vec3 direction = new Vec3(rayValue.x, rayValue.y, rayValue.z);
        if (direction.lengthSqr() < MIN_RAY_LENGTH_SQR) {
            throw new SpellRuntimeException(SpellRuntimeException.NON_POSITIVE_VALUE);
        }

        double distance = maxDistanceValue == null ? MAX_RANGE : maxDistanceValue.doubleValue();
        if (distance <= 0.0D) {
            throw new SpellRuntimeException(SpellRuntimeException.NON_POSITIVE_VALUE);
        }
        ...
        Vec3 start = new Vec3(positionValue.x, positionValue.y, positionValue.z);
