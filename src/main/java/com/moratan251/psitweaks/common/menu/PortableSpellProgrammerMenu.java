...
public final class PortableSpellProgrammerMenu extends AbstractContainerMenu {
    ...
    private final Spell initialSpell;

    public static PortableSpellProgrammerMenu fromNetwork(int id, Inventory inventory, RegistryFriendlyByteBuf buf) {
        return new PortableSpellProgrammerMenu(id, inventory, buf.readVarInt(), buf.readUUID(),
                Spell.STREAM_CODEC.decode(buf));
    }
